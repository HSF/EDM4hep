...
void writeRecoParticleRef(const TestState& testState) {
  ...
  refColl.setReferenceCollection();
  ...
  for (int iEvent = 0; iEvent < NEVENTS; ++iEvent) {
    ...
    for (const auto& ref : refColl) {
      ...
      ASSERT_EQUAL(expectedParticle, expectedParticle, "Wrong particle reference during creation");
      i++;
    }
...
void readRecoParticleRef(const TestState& testState) {
  ...
  for (int iEvent = 0; iEvent < NEVENTS; ++iEvent) {
    ...
    for (const auto& ref : refColl) {
      ...
      ASSERT_EQUAL(expectedReco, expectedReco, "Wrong particle in reference after reading");
      i++;
    }
